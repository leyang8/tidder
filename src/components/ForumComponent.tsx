...
const ForumComponent = ({ forumData }: ForumComponentProps) => {
  ...
  async function submitFollow(event: React.FormEvent) {
    const url = `http://localhost:5002/api/secure/follows/${Cookies.get(
      "currentUserID"
    )}/${forumData.creatorID}`;
    var data = {};
    if (!userFollows) {
    ...
    <Card className="max-w-5xl">
      ...
      {isAdmin == "true" && (
        <Card>
          <form onSubmit={handleDelete}>
            ...
            <Button
              className="mt-5 mb-5 max-w-xl transition-transform transform hover:scale-105"
              color="failure"
              type="submit"
            >
              Delete Forum
      ...
      <Card>
        <form onSubmit={submitReply}>
          ...
          <Button
            className="mt-5 max-w-xl transition-transform transform hover:scale-105"
